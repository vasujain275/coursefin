...
// ============================================================================
// Purpose: Bridge between MPV player and database for progress tracking
// Features:
...
func NewTracker(db *database.DB, player *mpv.Player) *Tracker {
	return &Tracker{
		db:                db,
		player:            player,
...
func (t *Tracker) trackLoop() {
	...
	for {
		...
		case <-ticker.C:
			t.mu.Lock()
			if t.tracking && t.player.IsRunning() {
